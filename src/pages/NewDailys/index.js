...
import { connect } from "../../redux/blockchain/blockchainActions5";
import { fetchData } from "../../redux/data/dataActions5";
...
function Index() {
  ...
  const blockchain = useSelector((state) => state.blockchain5);
  const data = useSelector((state) => state.data5);
  ...
  const claimNFTs = async () => {
    ...
    // Use live contract cost if available (prevents YOU_SEND_THE_WRONG_VALUE)
      ...
      // 2) Add buffer (20%)
      const gas = Math.ceil(Number(estimatedGas) * 0.9);
  ...
  const getConfig = async () => {
    const configResponse = await fetch("/config/config5.json", {
      headers: {
                      ...
                      >
                        <s.Container
                          ...
                          style={{
                            ...
                            backgroundImage: data && data.auctionURI ? `url(${(data.auctionURI)})` : '',
                            padding: 10,
                            borderRadius: '50%',
                          ...
                          {Number(data.totalSupply) >= CONFIG.MAX_SUPPLY ? (
                            ...
                            <>
                              ...
                              { data && data.auctionURI && <s.Container ai={"center"} jc={"center"} fd={"row"} style={{ paddingTop: '10px' }}>
                                <BiFullscreen style={{ cursor: 'pointer' }} onClick={() => { setIsOpenLightbox(true); setLightboxImage(`${(data.auctionURI)}`) }} />
                              </s.Container>}
                            </>
                          )}
                        </s.Container>
